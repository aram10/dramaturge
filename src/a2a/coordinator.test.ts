...
describe('Coordinator', () => {
  ...
  describe('task assignment', () => {
    it('assigns a navigation task to the scout agent', () => {
      ...
      const assignment = directives.find((d) => (d.data as any).type === 'task-assigned');
  ...
  describe('broadcastDirective', () => {
    ...
    it('uses coordinator role when fromAgentId is coordinator', () => {
      ...
      coordinator.broadcastDirective('coordinator', 'Redirect to forms');
    ...
    it('prevents metadata from overriding type and text', () => {
      ...
      coordinator.broadcastDirective('agent-reviewer', 'Focus on forms', {
        type: 'malicious',
        text: 'overridden',
      });
  ...
  describe('inherits Planner functionality', () => {
    ...
    it('can record dispatch and route followups (inherited from Planner)', () => {
      ...
      const item = coordinator.routeFollowup({ type: 'crud', reason: 'Test CRUD' }, 'node-1');
